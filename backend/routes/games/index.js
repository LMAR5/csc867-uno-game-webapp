...
router.post("/create", async (request, response) => {
  ...
  try {
    ...
    const {
      ...
      numberPlayers: finalNumberPlayers,
    } = await Games.create(creatorId, description, number_players);
...
router.get("/:id", async (request, response) => {
  ...
  try {
    ...
    if (!isPlayerInGame) {
      // Reload lobby page if player not in the game
      response.redirect("/lobby");
    }
...
router.post("/:id/play", async (request, response) => {
  ...
  try {
    ...
    if (turnUpdates > 1) {
      Games.updateGameCurrentTurn(gameId, userId).then((result) => {
        Games.updateGameCurrentTurn(gameId, userId).then((result2) => {
          return;
        });
      });
    } else {
      await Games.updateGameCurrentTurn(gameId, userId);
    }
